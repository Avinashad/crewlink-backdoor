...
-- Replaces worker_profiles.expertise_codes array
...
-- Backfill from existing expertise_codes array (text[])
INSERT INTO worker_profile_expertise (worker_user_id, expertise_code)
SELECT
  wp.user_id,
  code_val
FROM worker_profiles wp,
     unnest(wp.expertise_codes) AS code_val
WHERE wp.expertise_codes IS NOT NULL
  AND array_length(wp.expertise_codes, 1) > 0
...
'Junction: worker profiles ↔ expertise codes. Replaces expertise_codes array in worker_profiles.';
