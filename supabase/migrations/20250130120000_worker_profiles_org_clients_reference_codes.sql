...
-- Ensure RLS helper functions exist (from migration 009)
CREATE OR REPLACE FUNCTION public.is_active_org_member(_org_id uuid)
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.org_memberships WHERE org_id = _org_id AND user_id = auth.uid() AND COALESCE(status, 'active') = 'active');
$$;
CREATE OR REPLACE FUNCTION public.is_active_org_owner(_org_id uuid)
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.org_memberships WHERE org_id = _org_id AND user_id = auth.uid() AND role = 'owner' AND COALESCE(status, 'active') = 'active');
$$;
