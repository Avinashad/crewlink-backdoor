...
-- Now add the deferred RLS policy on training_module_items that references module_invitations
CREATE POLICY "Workers can view invited module items"
  ON training_module_items FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM module_invitations mi
      WHERE mi.module_id = training_module_items.module_id
        AND mi.worker_user_id = auth.uid()
        AND mi.status NOT IN ('revoked')
    )
  );
